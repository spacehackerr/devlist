...
+ четкое разделение интерфейса и реализации класса. У каждого объекта должно быть свое API (четко определенное)
+ У классов проводятся строгие архитектурные границы: между интерфейсом и реализацией класса, которые необходимо строго соблюдать
...
Таким образом, для взаимодействия с объектов наружу выставляется только спецификация (интерфейс) обхекта,
...
- (-) при возрастании иерархии, нужно помнить о проблеме хрупких базовых классов. Внесение изменений в базовый класс может сломать поведение дочерних. Предок должен быть закрыт от наследников настолько, насколько это возможно.
...
TellDon'tAsc - https://martinfowler.com/bliki/TellDontAsk.html
...
Цель - разбиения модуля на компоненты таким образом, чтобы каждый компонент имел единственную обязанность, и имел одну причину для изменения.
...
- Глвные моменты: Пред и пост условия. Пред условия не могут быть усилены, пост условия не могут быть ослаблены 
    - 1 Ковариантность возвращаемого значение (пост условие)
    - 2 Контрвариантность принимаемого значения (пред условие)
    - 3
     Ковариантность выбрасываемых исключений (пост условие). 
