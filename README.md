...
иерархию наследования. Поиск и правильное выстраивание отношений между объектами
...
— это модель объекта реального мира, в которой рассматриваются отдельные значимые характеристики (данные, поведение) для данного контекста, 
исключая незначемые.

В контексте ООП, абстракция проявляется в 2х видах :
1)	когда мы моделируем некий объект, мы можем отказаться от его частей (характеристик), не важных в контексте программы.
...
2)	когда мы имеем больше описания об объекте, чем нужно остальной части программы.
...
2)	в сокрытие ненужных деталей реализации от клиента, используя инкапсуляцию. Вся работа с объектом ведется только через интерфейс.
Главное, чтобы инициализированный объект выполнял возложенную работу, а как он это делает внутри - не важно (реализация скрывается от внешнего мира с помощью инкапсуляции). 
...
Механизм реализации: с использованием абстрактных классов и виртуальных методов.
    - абстрактным метод по умолчанию не имеет реализацию
    - виртуальный метод имеет базовую реализацию, которую можно переопределить в наследнике.
...
- следующий вариант более предпочтительный:  реализация интерфейса. целевой класс через параметр метода/конструктора принимает объект с логикой, расширяющей поведение этого класса. 
